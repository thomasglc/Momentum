  ...
  <div class="px-4 py-4 pb-6">
    ...
    <div class="grid grid-cols-4 gap-1 mb-4">
      <button
        ...
        :class="[getPhaseConfig(phase.id).bg, expanded === phase.id ? 'ring-2 ring-offset-1 opacity-100' : 'opacity-60']"
        @click="toggle(phase.id)"
      >
        <p class="text-[10px] font-bold uppercase tracking-wide" :class="getPhaseConfig(phase.id).text">Ph.{{ phase.id }}</p>
    ...
    <div class="flex flex-col gap-2">
      <div v-for="phase in phases" :key="phase.id" class="bg-white rounded-xl shadow-sm overflow-hidden">

        <!-- En-tête cliquable -->
        <button class="w-full text-left p-4 flex items-center gap-3" @click="toggle(phase.id)">
          <div class="w-2 self-stretch rounded-full flex-shrink-0" :class="getPhaseConfig(phase.id).bar"></div>
          <div class="flex-1 min-w-0">
            <p class="text-[10px] font-bold uppercase tracking-widest mb-0.5" :class="getPhaseConfig(phase.id).text">
        ...
        <div v-if="expanded === phase.id" class="border-t border-gray-100 px-4 pb-4 pt-3 flex flex-col gap-4">
          ...
          <div>
            ...
            <div class="flex flex-col gap-1.5">
              <div v-for="day in phase.weekStructure" :key="day.day" class="flex gap-3 items-start">
                <div
                  class="flex-shrink-0 w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-bold"
                  :class="DAY_TYPE_STYLES[day.type] ?? DAY_TYPE_STYLES.rest"
                >{{ day.day.slice(0, 2) }}</div>
                <div class="flex-1 min-w-0">
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-1.5">
              <button
                ...
                :class="[getPhaseConfig(phase.id).weekBtn, store.currentWeekNumber === wn ? 'ring-2' : '']"
                @click="goToWeek(wn)"
              >S{{ wn }}</button>
            </div>
          </div>
...
// Day type badge styles (specific to weekly structure display)
...
function phaseWeekNumbers(phaseId) {
  return planData.weeks.filter(w => w.phase === phaseId).map(w => w.weekNumber)
}
